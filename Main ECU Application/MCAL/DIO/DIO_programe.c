...
void MDIO_voidSetPortValue( u8 Copy_u8Port , u8 Copy_u8Position , u16 Copy_u8Value ){
	...
	case DIOA:
		...
		if      ( Copy_u8Position == DIO_LOW  ){ GPIOA->ODR = (GPIOA->ODR & 0xFF00 ) | ( Copy_u8Value ) ; }
		else if ( Copy_u8Position == DIO_HIGH ){ GPIOA->ODR = (GPIOA->ODR & 0x00FF ) | (     Copy_u8Value ) ; }

		break;
	case DIOB:

		if      ( Copy_u8Position == DIO_LOW  ){ GPIOB->ODR = (GPIOB->ODR & 0xFF00 ) | ( (u8)Copy_u8Value ) ; }
		else if ( Copy_u8Position == DIO_HIGH ){ GPIOB->ODR = (GPIOB->ODR & 0x00FF ) | (     Copy_u8Value ) ; }

		break;
	case DIOC:

		if      ( Copy_u8Position == DIO_LOW  ){ GPIOC->ODR = (GPIOC->ODR & 0xFF00 ) | ( (u8)Copy_u8Value ) ; }
		else if ( Copy_u8Position == DIO_HIGH ){ GPIOC->ODR = (GPIOC->ODR & 0x00FF ) | (     Copy_u8Value ) ; }
